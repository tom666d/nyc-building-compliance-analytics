...
| Property/compliance manager | Identify properties requiring action | open violations, active permit records, oldest unresolved item |
...
| Active permit record | distinct permit source payload mapped to `ISSUED` whose expiration date has not passed as of the snapshot date; this is not a unique permit count |
...
These definitions are hypotheses to validate with domain users, not universal facts. Live-source profiling invalidated the original permit key and the simple violation rule based only on disposition date. The implemented marts therefore expose a permit-source-record measure, map violation categories explicitly, and retain unmapped statuses for review.
